...
const faqs = [
  {
    question: "How is digital gold stored and secured? 🛡️",
  ...
  {
    question: "What is the Jewellery Cloud? ☁️",
        ...
        <Accordion className="w-full">
          {faqs.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`} className="border-slate-200">
              <AccordionTrigger className="text-slate-800 hover:text-brand-secondary hover:no-underline text-lg text-left py-6 transition-colors font-bold">
                {faq.question}
              </AccordionTrigger>
